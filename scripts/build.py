...
def generate_channel_page(channel):
    ...
    pip_style = ' style="display:none"' if channel.get('type') == 'youtube' else ''
    ...
    parts.append('      <button class="btn-pip" id="btn-pip" aria-label="Picture in Picture"' + pip_style + '><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M19 7V4a2 2 0 0 0-2-2H7a2 2 0 0 0-2 2v3m14 0v10a2 2 0 0 1-2 2h-2m0-10V7a2 2 0 0 1 2-2h2"></path></svg></button>')
    parts.append('    </div>')
    parts.append('  </div>')
    ...
    parts.append('    var pipBtn = document.getElementById("btn-pip"); if (pipBtn) { pipBtn.addEventListener("click", async function() { var video = document.getElementById("vid"); if (video !== document.pictureInPictureElement) { try { await video.requestPictureInPicture(); } catch (err) { console.error("PiP error:", err); } } else { await document.exitPictureInPicture(); } }); }')
